...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let minDisplayTime = 1200; // Minimum time to show loader (1.2s)
    ...
    function checkResourcesLoaded() {
        ...
        images.forEach(img => {
            if (img.complete) {
                ...
                img.addEventListener('load', function() {
                    imagesLoaded++;
                    updateLoaderProgress(imagesLoaded, imagesTotal);
        ...
        // Update progress if all images already loaded
        if (imagesLoaded === imagesTotal) {
            resourcesLoaded = true;
            return true;
        } else {
    ...
    function updateLoaderProgress(loaded, total) {
        if (!loaderProgressBar || total === 0) return;
        const percent = Math.min(95, (loaded / total) * 100);
        loaderProgressBar.style.width = percent + '%';
        
        // Update ARIA attributes
        const progressContainer = document.querySelector('.loader-progress[role="progressbar"]');
        if (progressContainer) {
            progressContainer.setAttribute('aria-valuenow', Math.round(percent));
        }
    }

        // Smooth hide loader with fade out
    function hideLoader(skipAnimation = false) {
        if (loaderHidden || !loader) return;
        loaderHidden = true;

        // Save that user has seen the loader
        ...
        if (skipAnimation) {
            ...
            // Add fade out class for first-time users
    ...
    // Try to hide loader when resources are ready and minimum time passed
    ...
    if (loader) {
        // If user has seen loader before, skip it immediately
        if (hasSeenLoader) {
            hideLoader(true);
        } else {
            ...
            // Fallback: hide loader after window load (all resources including external)
            window.addEventListener('load', function() {
                resourcesLoaded = true;
                tryHideLoader();
            }, { once: true });

            // Safety fallback: hide after max time (4 seconds)
            setTimeout(() => {
                if (!loaderHidden) {
                    resourcesLoaded = true;
                    hideLoader();
            ...
            // Allow click to skip (useful for development)
            ...
            loader.setAttribute('title', 'Kliknij aby pominąć');
        }
    }
